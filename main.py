...
def qr_default() -> List[Dict]:
    return [
        ...
        {"action": "message", "label": "학년/반 변경", "messageText": "학년 변경 2 8"},
    ]
...
async def webhook(request: Request, x_kakao_signature: str = Header(None)):
    ...
    # 학년/반 설정
    if t.startswith("학년 변경") or t.startswith("학년/반"):
        parts = t.split()
        if len(parts)>=3 and parts[1].isdigit() and parts[2].isdigit():
            ...
            return kakao_simple_text("변경할 학년과 반을 입력해주세요. 예: `학년 변경 2 8`", qr_default())
    ...
    return kakao_simple_text(
        "무엇을 도와드릴까요?\n가능한 명령: `오늘 시간표`, `내일 시간표`, `오늘 급식`, `9월3일 급식`, `이번 주 학사일정`, `이번 달 학사일정`, `학년 변경 2 8`",
        qr_default()
    )
